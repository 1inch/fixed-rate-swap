...
contract('FixedFeeSwap', function ([wallet1, wallet2]) {
    ...
    afterEach(async function () {
        for (const addr of [wallet1, wallet2]) {
            ...
            if (!p.isZero()) {
                await this.fixedRateSwap.withdraw(p, { from: addr });
            }
            for (const token of [this.USDT, this.USDC]) {
    ...
    describe('Arbitrary withdrawal', async function () {
        ...
        it('should be equal to (withdraw + swap) {1:9}', async function () {
            // arbitrary withdraw
            const arbitraryBalances = await this.fixedRateSwap.contract.methods.withdrawWithRatio(ether('1'), ether('0.1')).call();
        ...
        it('should be equal to (withdraw + swap) {9:1}', async function () {
            // arbitrary withdraw
            const arbitraryBalances = await this.fixedRateSwap.contract.methods.withdrawWithRatio(ether('1'), ether('0.9')).call();
        ...
        it('should be equal to (withdraw + swap) {1:1}', async function () {
            // arbitrary withdraw
            const arbitraryBalances = await this.fixedRateSwap.contract.methods.withdrawWithRatio(ether('1'), ether('0.5')).call();
    ...
    describe('Arbitrary deposit', async function () {
        it('should be equal to (swap + deposit) {balances, deposit} = {(0,100), (1,0)}', async function () {
            await this.fixedRateSwap.deposit('0', ether('100'));
            ...
            // arbitrary deposit
            const lpBalanceWallet = await this.fixedRateSwap.contract.methods.deposit(ether('1'), 0).call();
            const arbitraryLpBalance = web3.utils.toBN(lpBalanceWallet);

            // swap + deposit
            const outputAmount = await this.fixedRateSwap.contract.methods.swap0To1(ether('1')).call();
            await this.fixedRateSwap.swap0To1(ether('1'));
            const lpBalance = await this.fixedRateSwap.contract.methods.deposit(outputAmount, 0).call();
        ...
        it('should be equal to (swap + deposit) {balances, deposit} = {(100,0), (0,1)}', async function () {
            await this.fixedRateSwap.deposit(ether('100'), '0');
            ...
            // arbitrary deposit
            const lpBalanceWallet = await this.fixedRateSwap.contract.methods.deposit(0, ether('1')).call();
            const arbitraryLpBalance = web3.utils.toBN(lpBalanceWallet);

            // swap + deposit
            const outputAmount = await this.fixedRateSwap.contract.methods.swap1To0(ether('1')).call();
    ...
    describe('Deposits', async function () {
        it.skip('should be cheap', async function () {
            await this.fixedRateSwap.deposit(ether('1'), ether('1'));
            await this.fixedRateSwap.deposit(ether('0.5'), ether('1'));
            await this.fixedRateSwap.deposit(ether('1'), ether('0.5'));
            await this.fixedRateSwap.deposit(ether('1'), ether('0'));
            const receipt = await this.fixedRateSwap.deposit(ether('0'), ether('1'));
        ...
        describe('mint lp', async function () {
            ...
            tests.forEach(test => {
                it(`should mint ${test[4]} lp when {balances, deposit} = {(${test[0]}, ${test[1]}), (${test[2]}, ${test[3]})}`, async function () {
                    ...
                    await this.fixedRateSwap.deposit(usdtBalance, usdcBalance);
        ...
        it('should be denied for zero amount', async function () {
            await expectRevert(
                this.fixedRateSwap.deposit(ether('0'), ether('0')),
    ...
    describe('Withdrawals', async function () {
        ...
        it('should be able to withdraw fully', async function () {
            await this.fixedRateSwap.withdraw(ether('2'));
        ...
        describe('ratio withdrawals', async function () {
            const tests = [
                ['1', '1', '0', '0'],
                ['1', '9', '4146957397460', '37325783647286'],
                ['9', '1', '37325783647286', '4146957397460'],
            ...
            tests.forEach(test => {
                it(`should be able to withdraw with ${test[0]}:${test[1]}`, async function () {
                    ...
                    await this.fixedRateSwap.withdrawWithRatio(withdrawalAmount, token0Amount);
        ...
        it('should revert when withdraw too much', async function () {
            await expectRevert(
                this.fixedRateSwap.withdraw(ether('3')),
        ...
        it('should revert when withdraw too much in one token', async function () {
            await expectRevert(
                this.fixedRateSwap.withdrawWithRatio(ether('2'), ether('0')),
                'Amount exceeds total balance',
            );
        });

        it('should withdraw all after swap', async function () {
            await this.fixedRateSwap.swap0To1(swapAmount, { from: wallet2 });
            await this.fixedRateSwap.withdraw(ether('2'));
    ...
    describe('Swaps', async function () {
        ...
        it('should swap directly', async function () {
            await this.fixedRateSwap.swap0To1(swapAmount);
        ...
        it('should swap inversly', async function () {
            await this.fixedRateSwap.swap1To0(swapAmount);
